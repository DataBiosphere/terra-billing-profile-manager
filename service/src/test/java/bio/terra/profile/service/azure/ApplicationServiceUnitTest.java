...
class ApplicationServiceUnitTest extends BaseUnitTest {
  ...
  void addTagToMrg() {
    ...
    var resourceUuid = UUID.randomUUID().toString();
    when(resourceGroup.id()).thenReturn(resourceUuid);
    ...
    verify(crlService)
        .updateTagsForResource(tenantId, subId, resourceUuid, Map.of("fake_tag", "fake_value"));
  }
